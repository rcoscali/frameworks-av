...
namespace android 
{
				    ...
				    const KeyedVector<String8, String8> *headers) 
  {
	...
	source = new HTTPLiveSource(notify, url, headers, mUIDValid, mUID);
      ...
      {
	ALOGV("NuPlayer::setDataSourceAsync - Dash source to be created\n");
  ...
  void NuPlayer::onMessageReceived(const sp<AMessage> &msg) 
  {
    switch (msg->what()) 
	  ...
	  mDeferredActions.push_back(new SimpleAction(&NuPlayer::performDecoderShutdown));

	  sp<RefBase> obj;
	  CHECK(msg->findObject("native-window", &obj));

	  mDeferredActions.push_back(new SetSurfaceAction(static_cast<NativeWindowWrapper *>(obj.get())));
	  ...
	  if (what == ACodec::kWhatFillThisBuffer) 
	    {
	      status_t err = feedDecoderInputData(audio, codecRequest);
		  ...
		  if (mVideoDecoder == NULL &&
		  ...
		  else 
		    {
                    ...
                    if (!codecRequest->findInt32("channel-mask", &channelMask)) 
		      {
                    ...
                    CHECK_EQ(mAudioSink->open(sampleRate,
					      numChannels,
	      ...
	      else 
		{
		  ...
		  CHECK(codecRequest->findRect("crop",
					       &cropLeft, 
					       &cropTop, 
					       &cropRight, 
					       &cropBottom));
		  ...
		  if (videoInputFormat->findInt32("sar-width", &sarWidth) &&
		      videoInputFormat->findInt32("sar-height", &sarHeight)) 
		    {
	  ...
	  if (what == Renderer::kWhatEOS) 
	      ...
	      else 
		{
		  ...
		  notifyListener(MEDIA_ERROR, MEDIA_ERROR_UNKNOWN, finalResult);
                }
	  ...
	  mDeferredActions.push_back(new SimpleAction(&NuPlayer::performDecoderFlush));
	...
	else if (err != OK) 
	  {
                ...
                if (formatChange || timeChange) 
		  {
                    if (mFlushingAudio == NONE && mFlushingVideo == NONE) 
		      {
                        ...
                        // flushing.
                        mDeferredActions.push_front(new SimpleAction(&NuPlayer::performScanSources));
		      }
  ...
  void NuPlayer::renderBuffer(bool audio, const sp<AMessage> &msg) 
  {
    ...
    int64_t &skipUntilMediaTimeUs = audio
      ? mSkipRenderingAudioUntilMediaTimeUs
      : mSkipRenderingVideoUntilMediaTimeUs;
  ...
  status_t NuPlayer::setVideoScalingMode(int32_t mode) 
  {
    ...
    if (mNativeWindow != NULL) 
      {
        status_t ret = native_window_set_scaling_mode(mNativeWindow->getNativeWindow().get(), 
						      mVideoScalingMode);
        if (ret != OK) 
	  {
