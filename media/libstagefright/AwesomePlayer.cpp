...
namespace android {
  ...
  status_t AwesomePlayer::setDataSource_l(const sp<DataSource> &dataSource) {
    ...
    ALOGV("AwesomePlayer::setDataSource_l - drmFlag = %d", extractor->getDrmFlag());
  ...
  void AwesomePlayer::checkDrmStatus(const sp<DataSource>& dataSource) {
    ALOGV("AwesomePlayer::checkDrmStatus - Enter: calling getDrmInfo");
    dataSource->getDrmInfo(mDecryptHandle, &mDrmManagerClient);
    if (mDecryptHandle != NULL) {
      CHECK(mDrmManagerClient);

      ALOGV("AwesomePlayer::checkDrmStatus - DecryptHandle status = %d", mDecryptHandle->status);
