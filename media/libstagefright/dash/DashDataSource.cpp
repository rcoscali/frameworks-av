...
namespace android {
  ...
  status_t DashDataSource::initCheck() const {
    return OK;
  }

  size_t DashDataSource::countQueuedBuffers() {
    Mutex::Autolock autoLock(mLock);

    return mBufferQueue.size();
  }

  ssize_t DashDataSource::readAtNonBlocking(
					    off64_t offset, void *data, size_t size) {
    Mutex::Autolock autoLock(mLock);

    if (offset != mOffset) {
      ALOGE("Attempt at reading non-sequentially from DashDataSource.");
      return -EPIPE;
    }
  ...
  ssize_t DashDataSource::readAt(off64_t offset, void *data, size_t size) {
    Mutex::Autolock autoLock(mLock);
    return readAt_l(offset, data, size);
  }

  ssize_t DashDataSource::readAt_l(off64_t offset, void *data, size_t size) {
    if (offset != mOffset) {
      ALOGE("Attempt at reading non-sequentially from DashDataSource.");
      return -EPIPE;
    }
  ...
  void DashDataSource::queueBuffer(const sp<ABuffer> &buffer) {
    Mutex::Autolock autoLock(mLock);
  ...
  void DashDataSource::queueEOS(status_t finalResult) {
