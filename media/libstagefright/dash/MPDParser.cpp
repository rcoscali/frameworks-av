...
namespace android {
  ...
  */
  ...
  mpdparser_parse_descriptor_type_node (vector<MPDParser::MPDDescriptorType>* list, xmlNode *a_node)
  {
	...
	mpdparser_get_xml_prop_string (a_node, "default_KID", &new_descriptor->mDefaultKID);
	xmlBufferPtr bufptr = xmlBufferCreate();
	for (cur_node = a_node->children; cur_node; cur_node = cur_node->next)
	  {
	    xmlBufferEmpty(bufptr);
	    xmlNodeDump(bufptr, a_node->doc, cur_node, 1, 1);
	    AString content((const char *)xmlBufferContent(bufptr));
	    content.trim();
	    if (content.size() > 0)
	      {
		ALOGV(" - Adding child node content: '%s'\n", content.c_str());
		new_descriptor->mChildrenNodes.push_back(content);
	      }
	    else
	      ALOGI(" - Discard empty child !!\n");
	  }
  ...
  mpdparser_get_xml_prop_string_vector_type (xmlNode *a_node, const char *property_name, uint32_t *property_size)
  {
	...
	xmlFree (prop_string);
      ...
      ALOGW(" - Empty vector for property %s %s\n", property_name, prop_string);
